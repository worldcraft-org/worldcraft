...
def generate_point_cloud(
    ...
    # 1. ADD THIS ARGUMENT
    semantic_output_name: Optional[str] = "semantics",
    crop_obb: Optional[OrientedBox] = None,
    std_ratio: float = 10.0,
) -> Tuple[o3d.geometry.PointCloud, Optional[torch.Tensor]]:
        ...
        while not progress_bar.finished:
            ...
            if semantic_output_name is not None:
                if semantic_output_name not in outputs:
                    CONSOLE.rule("Error", style="red")
                    CONSOLE.print(f"Could not find {semantic_output_name} in outputs", justify="center")
                    sys.exit(1)
                
                # Assuming outputs are logits (N, num_classes), take argmax to get class ID
                # If your model outputs straight labels, remove the argmax.
                raw_sem = outputs[semantic_output_name]
                if raw_sem.shape[-1] > 1:
                    semantic = torch.argmax(raw_sem, dim=-1, keepdim=True)
                else:
                    semantic = raw_sem
    ...
    if len(semantics) > 0:
        semantics = torch.cat(semantics, dim=0).cpu()
    else:
        semantics = None
    ...
    if remove_outliers:
        ...
        if ind is not None:
            ...
            # 8. APPLY OUTLIER MASK TO SEMANTICS
    ...
    # 9. RETURN TUPLE INSTEAD OF JUST PCD
