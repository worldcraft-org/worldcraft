...
def save_ply_with_semantics(pcd, semantics, filename):
    """Saves an Open3D point cloud + semantic tensor to a PLY file."""
    ...
    if has_semantics:
        semantics = semantics.numpy().flatten()
    ...
    for i in range(len(points)):
        ...
        if has_semantics:
            data.append(int(semantics[i]))
        vertex_data.append(tuple(data))
    ...
    if has_semantics:
        dtypes.append(('semantic', 'u2')) 

    vertex_array = np.array(vertex_data, dtype=dtypes)
...
class ExportPointCloud(Exporter):
    ...
    # 1. ADD THIS NEW ARGUMENT
    semantic_output_name: str = "semantics"
    """Name of the semantic output."""
    ...
    def main(self) -> None:
        ...
        pcd, semantics = generate_point_cloud(
            pipeline=pipeline,
            ...
            semantic_output_name=self.semantic_output_name,
            crop_obb=crop_obb,
        ...
        """
        save_ply_with_semantics(pcd, semantics, str(self.output_dir / "point_cloud.ply"))
